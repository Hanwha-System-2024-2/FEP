...
void save_order_to_file(fkq_order *order, char filepath[256]) {
        ...
        // fwrite(pad_and_save(order->hdr.tr_id,sizeof(order->hdr.tr_id)) , sizeof(order->hdr.tr_id), 1, file);        
        // pad_and_save(order->hdr.tr_id, sizeof(order->hdr.tr_id), file);
        ...
        // fwrite(pad_and_save(order->stock_name, sizeof(order->stock_name)), sizeof(order->stock_name), 1, file);
        ...
        for (int i = 1; i < MAX_CLIENTS; i++) {
            if (fds[i].fd != -1 && (fds[i].revents & POLLIN)) {
                ...
                // } else if (bytes_received == sizeof(received_order.hdr.length)) {
                    ...
                    save_order_to_file(&received_order, filepath);
                    w_count->wc++;
